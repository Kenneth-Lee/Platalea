...
HELP_TOPICS = frozenset({"serve", "board", "gpg", "file", "config", "service", "discover", "power"})
...
分组帮助: {CLI_NAME} help [serve|board|gpg|file|config|service|discover|power]
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(
        f"运行 {CLI_NAME} help 查看全部命令，或 {CLI_NAME} help [serve|board|gpg|file|config|service|discover|power]。",
        file=sys.stderr,
    )
    return 2
