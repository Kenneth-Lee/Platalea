...
from .discover_cmd import run_discover
from .gpg_cmd import (
    run_list_keys,
    run_pass_decrypt,
    run_pass_encrypt,
...
GROUP_COMMANDS = frozenset({"gpg", "file", "config", "service", "help"})
...
HELP_TOPICS = frozenset({"serve", "board", "gpg", "file", "config", "service", "discover"})
...
def _cmd_status(args: argparse.Namespace) -> int:
    cfg_path = _resolved_config(args.config)
    config, _agent_config = load_config(cfg_path)
    ...
    if config.max_import_bytes is None:
        print("Max import bytes: unlimited")
    else:
        print(f"Max import bytes: {config.max_import_bytes}")
    return 0
...
def _service_help() -> str:
        return """系统服务控制（跨平台统一接口）:
    service install [--config PATH]
                                                安装并启用开机自动拉起（owner=当前用户）
    service uninstall [--keep-state]
                                                卸载托管服务（可选保留控制面状态）
    service status
                                                查看安装状态、owner 与托管单元信息"""


def _discover_help() -> str:
        return f"""家庭网络设备发现:
    discover [--service-type {"_localmanager._tcp.local."}] [--timeout SEC] [--json]
                                                列出当前局域网内提供家庭网络服务的设备"""
...
分组帮助: {CLI_NAME} help [serve|board|gpg|file|config|service|discover]
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(
        f"运行 {CLI_NAME} help 查看全部命令，或 {CLI_NAME} help [serve|board|gpg|file|config|service|discover]。",
        file=sys.stderr,
    )
    return 2
