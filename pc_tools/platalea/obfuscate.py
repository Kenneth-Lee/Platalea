...
def _run_mode(mode: str, argv: list[str] | None) -> int:
    ap = argparse.ArgumentParser(
        ...
        description="与 Local Manager 兼容的快速混淆/反混淆（.qx 格式）",
    )
    ap.add_argument("input", type=Path, help="输入文件或目录")
    ap.add_argument("-p", "--password", required=True, help="密码")
    ap.add_argument("-o", "--output", type=Path, default=None, help="输出文件或目录")
    ap.add_argument("--in-place", action="store_true", help="原地修改（仅单文件）")
    ...
    if inp.is_file():
        return _process_single_file(inp, password, out_base, is_obfuscate, args.in_place, args.quiet)

    if args.in_place:
        print("错误：--in-place 仅支持单文件", file=sys.stderr)
        return 1

    out_dir = out_base if (out_base and out_base.is_dir()) else (
        out_base if out_base else inp.parent / (inp.name + "_out")
    )
