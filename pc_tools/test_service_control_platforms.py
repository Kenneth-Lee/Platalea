...
class WindowsServiceAdapterTest(unittest.TestCase):
    ...
    def test_install_user_server_uses_runas_service(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp, mock.patch(
            ...
            adapter.install_user_server_unit(spec)
            self.assertTrue(any(cmd[:3] == ["sc.exe", "create", WINDOWS_BOOTSTRAP_LABEL] for cmd in calls))
            self.assertTrue(any(any(item.startswith("obj= .\\kenny") for item in cmd) for cmd in calls))
            self.assertTrue(any(any(item.startswith("password= secret-pass") for item in cmd) for cmd in calls))
    ...
    def test_query_status_sets_windows_platform(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp, mock.patch(
            ...
            self.assertTrue(any(cmd[:3] == ["sc.exe", "query", WINDOWS_PRIVILEGED_LABEL] for cmd in calls))
            self.assertTrue(any(cmd[:3] == ["sc.exe", "query", WINDOWS_BOOTSTRAP_LABEL] for cmd in calls))
