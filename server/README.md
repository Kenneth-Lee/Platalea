...
lmserver post default "Hello"
lmserver create-board "厨房留言"    # 需 host 密码
```

连接远程设备时指定 `--host`，不会自动启动本机服务：

```bash
lmserver --host 192.168.1.10 list-boards
```
...
| 路径 | 作用 |
|------|------|
| `lmserver/` | **pip 包**：CLI、服务、存储、Agent |
| `pyproject.toml` | pip 安装入口（包名 `lmserver`，命令 `lmserver`） |
| `bulletin_server.py` | 兼容包装（转发到 `lmserver`） |
| `board_client.py` | 兼容包装（转发到 `lmserver`） |
